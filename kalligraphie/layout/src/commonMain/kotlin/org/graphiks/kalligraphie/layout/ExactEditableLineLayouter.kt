...
public object ExactEditableLineLayouter : EditableLineLayouter {
     ...
     * validates final glyph-materialization routes before this method publishes an
     * [EditableLineResult]. The supplied resolver remains borrowed by the caller; all temporary
     * assets are closed before this method returns. A matching operation-local route proof may
     * avoid resolving an unchanged glyph twice, but it retains neither an asset nor representation
     * data and cannot escape the call. Failure and cancellation never publish a partial line.
     */
    public fun layout(request: MultiFontEditableLineRequest): EditableLineResult {
        ...
        fun perform(): EditableLineResult {
            ...
            return when (val resolved = FontFallbackResolver.resolve(request, proofs, context, pool)) {
     ...
     */
    override fun layout(request: EditableLineRequest): EditableLineResult = layoutOwned(
        request,
    ...
    ): EditableLineResult = layoutOwned(
        request,
        proofs,
        EditorOperationContext.create(EditorOperationProfile.unbounded, request.cancellationToken),
    )
    ...
    ): EditableLineResult = layoutOwned(request, GlyphMaterializationProofs(), context)
    ...
    private fun layoutOwned(
        ...
        var closeDiagnostics = emptyList<org.graphiks.kalligraphie.api.FontDiagnostic>()
        val result = try {
            layout(request, proofs, context, pool)
        } finally {
            closeDiagnostics = pool.close()
        }
        return result.afterClosure(pool.closeFailure, closeDiagnostics)
    ...
    internal fun layout(
        ...
        val certification = when (val result = certifyFinalGlyphs(request, placements, proofs, pool)) {
                    ...
                    for (profile in materialization.requirements.acceptedProfiles) {
                        ...
                        when (val acquired = pool.acquire(instance, profileMaterialization, profile)) {
                            ...
                            is FontOperationResult.Failure -> {
                                val acquiredError = acquired.error
                                val failure = CertificationResult.Failure(
                                    if (acquiredError is org.graphiks.kalligraphie.api.FontError.EditorOperationLimitExceeded) {
                                        EditableLineError.OperationLimitExceeded(acquiredError.exceeded)
                                    } else {
...
private fun EditableLineResult.afterClosure(
    closeFailure: org.graphiks.kalligraphie.api.FontError?,
    diagnostics: List<org.graphiks.kalligraphie.api.FontDiagnostic>,
): EditableLineResult {
    val closeDiagnostics = diagnostics.map(::fontDiagnostic)
    return when (this) {
        is EditableLineResult.Success -> if (closeFailure == null) {
            this
        } else {
            EditableLineResult.Failure(
                EditableLineError.FontMaterializationFailure(closeFailure),
                closeDiagnostics,
            )
        }
        is EditableLineResult.Failure -> EditableLineResult.Failure(error, this.diagnostics + closeDiagnostics)
        is EditableLineResult.Cancelled -> EditableLineResult.Cancelled(this.diagnostics + closeDiagnostics)
    }
}
